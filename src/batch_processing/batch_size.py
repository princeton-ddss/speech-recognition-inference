...
def calculate_batch_size(
    max_file_matrix_size_mb=20,
    remaining_proportion=0.6,
    device=None,
    total_memory_gb: Optional[int] = None,
):
    """
    Dynamically calculate the batch size for processing based on available GPU memory.

    Args:
        max_file_matrix_size_mb (int, optional): Maximum size of the file matrix in MB. Defaults to 20.
        remaining_proportion (float, optional): Proportion of GPU memory to keep free (not used for batch processing). Defaults to 0.6.
        device (str, optional): Device to run the processing on (e.g., 'cpu' or 'cuda'). Defaults to None.
        total_memory_gb (int, optional): Total GPU memory available in GB. If
        not provided, it will be determined automatically. Defaults to None.
    ...
    if batch_size < 1:
        raise Exception(
            "The current gpu memory is not enough to run the         current"
            " transcription model. Please either increase gpu memory or                "
            "         downsize the model"
        )
    return batch_size
